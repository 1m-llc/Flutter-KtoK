...
class Routes {
  ...
  static void configureRoutes(Router router) {
    ...
    router.define(blankRoute, handler: handler(BlankPage()),);

    /// トップページ
    router.define(rootRoute, handler: handler(MainPage()),);
    /// サイドメニュー
    router.define(aboutKtoKRoute, handler: handler(AboutKtoKPage()),);
    router.define(linkCollectionRoute, handler: handler(LinkCollectionPage()),);
    /// デザイン
    router.define(themeDataChangeRoute, handler: handler(ThemeDataChangePage()),);
    router.define(lightThemeDataRoute, handler: handler(LightThemeDataPage()),);
    router.define(darkThemeDataRoute, handler: handler(DarkThemeDataPage()),);
    router.define(appIconSetupRoute, handler: handler(AppIconSetupPage()),);
    router.define(lunchScreenSetupRoute, handler: handler(LunchScreenSetupPage()),);
    /// テスト
    router.define(unitTestRoute, handler: handler(UnitTestPage()),);
    router.define(widgetTestRoute, handler: handler(WidgetTestPage()),);
    router.define(integrationTestRoute, handler: handler(IntegrationTestPage()),);
  }
